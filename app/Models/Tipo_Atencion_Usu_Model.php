...
class Tipo_Atencion_Usu_Model extends BaseModel
{
    ...
    public function Listar_Tipo_Atencion_act_coordenadas($idTipoAtencion=null)
    {
        ...
        $builder->select('a_usu.act_coordenadas, a_usu.act_pro_int, a_usu.tipo_aten_id, a_usu.tipo_aten_nombre');
    ...
    public function Listar_Tipo_Atencion_filtro()
    {
        $db = \Config\Database::connect();
        $builder = $db->table('public.sgc_tipoatencion_usu as a_usu');
        $builder->select('a_usu.act_pro_int,a_usu.act_coordenadas,a_usu.tipo_aten_id, a_usu.tipo_aten_nombre');
        $builder->select("CASE WHEN a_usu.tipo_aten_borrado = 'f' THEN 'Activo' ELSE 'Inactivo' END as borrado");
    ...
    public function Listar_Tipo_Atencion_Sin_Formacion()
    {
        $db = \Config\Database::connect();
        $builder = $db->table('public.sgc_tipoatencion_usu as a_usu');
        $builder->select('a_usu.acc_participantes,a_usu.act_coordenadas, a_usu.act_pro_int, a_usu.tipo_aten_id, a_usu.tipo_aten_nombre');
        $builder->select("CASE WHEN a_usu.tipo_aten_borrado = 'f' THEN 'Activo' ELSE 'Inactivo' END as borrado");
        $builder->where('a_usu.tipo_aten_borrado', 'false');
    ...
    public function Listar_Tipo_Atencion_edit()
    {
        $db = \Config\Database::connect();
        $builder = $db->table('public.sgc_tipoatencion_usu as a_usu');
        $builder->select('a_usu.organismo_pp,a_usu.env_correo,a_usu.act_coordenadas,a_usu.acc_participantes, a_usu.act_pro_int, a_usu.tipo_aten_id, a_usu.tipo_aten_nombre');
        $builder->select("CASE WHEN a_usu.tipo_aten_borrado = 'f' THEN 'Activo' ELSE 'Inactivo' END as borrado");
        $query = $builder->get();
        $resultado = $query->getResult();
        return $resultado;
