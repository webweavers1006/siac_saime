...
<div class="row justify-content-center">
  <?php
  if (array_key_exists('mensajes', $mensajes) && !empty($mensajes['mensajes'])) {
  ?>
    <div class="col-lg-12 custom-col-width">
      <div class="card card_table " style="max-width: 1350px; margin: 20px auto;">
        ...
        foreach ($mensajes['mensajes'] as $mensaje) {
          $options[$mensaje['opcion']][$mensaje['mensaje']][] = $mensaje; 
        }
        foreach ($options as $opcion => $mensajes_grupo) {
          ?>
          <h2><?= $opcion ?></h2> 
          <table class="table table-striped <?= ($opcion == 'SOLICITUD DE CITA') ? 'ant-alert-info' : (($opcion == 'OBSERVACION INTERNA') ? 'ant-alert-success' : '') ?>">
            <thead>
              <tr>
                <th>Texto</th>
                <th>Acciones</th>
            ...
            <tbody>
              <?php
              foreach ($mensajes_grupo as $mensaje_texto => $mensajes) {
