...
class Audiencias_Controler extends BaseController
{
	...
	public function vista_solicitudes()
	{
		...
		$ch = curl_init("http://172.16.0.46:70/solicitudes/workers/".$userdata["iduser"]);
		...
		if ($datos['solicitudes']) 
		{
			...
			$ch = curl_init("http://172.16.0.46:70/solicitudes/".$id_requerimiento);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_HEADER, 0);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array(
				'Content-Type: application/json',
...
public function casos_categorias()
{
	...
	// Realiza la solicitud a la API para obtener los datos
	$casos = json_decode(file_get_contents("http://172.16.0.46:70/solicitudes/byCategorias", false, $contexto), true);
	$data['casos'] = $casos;
